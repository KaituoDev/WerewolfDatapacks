...
execute store result score %success ww.var run function ww:book/build/pairing/_if_already_selected with storage ww:temp validateInput
execute if score %success ww.var matches 1 run return 0

# check if current iteration player (uuid) has been taken
execute store result score %success ww.var run function ww:book/build/pairing/_if_already_taken with storage ww:temp validateInput
execute if score %success ww.var matches 1 run return 0

# check if current iteration player (uuid) has been taken
execute store result score %success ww.var run function ww:book/build/pairing/_if_already_dead with storage ww:temp validateInput
execute if score %success ww.var matches 1 run return 0
...
function ww:book/build/pairing/_set_trigger_command with storage ww:temp setTriggerCommandInput

data modify storage ww:temp lineInput.uuid set from entity @s UUID
function ww:book/build/pairing/_line with storage ww:temp lineInput
