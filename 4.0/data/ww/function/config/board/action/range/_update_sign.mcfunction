...
$execute if score %count ww.var matches 0 at @n[nbt={UUID:$(origin)}] run return run \
  ...
  '{"text":"已關閉","color":"red","clickEvent":{"action":"run_command","value":"function ww:config/board/action/_run {func: \\"ww:config/board/action/_range\\", args: {name: \\"$(name)\\", pos: \\"$(pos)\\"}}"}}'

$execute if score %count ww.var matches 1.. at @n[nbt={UUID:$(origin)}] run return run \
  data modify block $(pos) front_text.messages[2] set value \
  '{"score":{"name":"%count","objective":"ww.var"},"bold":true,"color":"green","clickEvent":{"action":"run_command","value":"function ww:config/board/action/_run {func: \\"ww:config/board/action/_range\\", args: {name: \\"$(name)\\", pos: \\"$(pos)\\"}}"}}'