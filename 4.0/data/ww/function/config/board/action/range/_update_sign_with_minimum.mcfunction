...
$execute if score %count ww.var matches 0 at @n[nbt={UUID:$(origin)}] run return run \
  ...
  '{"text":"已关闭","color":"red","clickEvent":{"action":"run_command","value":"function ww:config/board/action/_run {func: \\"ww:config/board/action/_range_min\\", args: {name: \\"$(name)\\", pos: \\"$(pos)\\", min: $(min)}}"}}'
