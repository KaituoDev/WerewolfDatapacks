...
$execute at @n[nbt={UUID:$(origin)}] run data merge block ^-1 ^ ^1 { \
  ...
  '{"text":"上一页","color":"gold","bold":true,"clickEvent":{"action":"run_command","value":"/function ww:config/board/action/_run {func: \\"ww:config/board/page/pool/_bads\\", args: {pos: \\"^-1 ^ ^1\\"}}"}}', \
  '{"text":""}', \
  '{"text":""}'], color:"white"} \
}
...
$execute at @n[nbt={UUID:$(origin)}] run data merge block ^2 ^ ^1 { \
  front_text:{has_glowing_text:0b, messages: ['{"text":""}', \
  '{"text":"重置狼人池设定","color":"red","bold":true,"clickEvent":{"action":"run_command","value":"/function ww:config/board/action/_run {func: \\"ww:config/board/action/pool/_reset_campaign\\", args: {campaign: \\"bads\\", page: \\"_bads_2\\"}}"}}', \
  '{"text":""}', \
  '{"text":""}'], color:"white"} \
}
$execute at @n[nbt={UUID:$(origin)}] run data merge block ^2 ^-1 ^1 { \
