...
$execute at @n[nbt={UUID:$(origin)}] run data merge block ^-1 ^ ^1 { \
  ...
  '{"text":"上一頁","color":"gold","bold":true,"clickEvent":{"action":"run_command","value":"/function ww:config/board/action/_run {func: \\"ww:config/board/page/pool/_goods\\", args: {pos: \\"^-1 ^ ^1\\"}}"}}', \
  '{"text":""}', \
  '{"text":""}'], color:"white"} \
}
...
# Center
$execute at @n[nbt={UUID:$(origin)}] run \
  function ww:config/board/action/pool/_update_sign {id: 'detective', origin: $(origin), pos: '^ ^2 ^1'}
$execute at @n[nbt={UUID:$(origin)}] run \
...
$execute at @n[nbt={UUID:$(origin)}] run \
  function ww:config/board/action/pool/_update_sign {id: 'villager', origin: $(origin), pos: '^2 ^1 ^1'}

$execute at @n[nbt={UUID:$(origin)}] run data merge block ^2 ^ ^1 { \
  front_text:{has_glowing_text:0b, messages: ['{"text":""}', \
  '{"text":"重置人類池設定","color":"red","bold":true,"clickEvent":{"action":"run_command","value":"/function ww:config/board/action/_run {func: \\"ww:config/board/action/pool/_reset_campaign\\", args: {campaign: \\"goods\\", page: \\"_goods_2\\"}}"}}', \
  '{"text":""}', \
  '{"text":""}'], color:"white"} \
}
$execute at @n[nbt={UUID:$(origin)}] run data merge block ^2 ^-1 ^1 { \
