...
$execute at @n[nbt={UUID:$(origin)}] run \
  function ww:config/board/action/pool/_update_sign {id: 'wraith', origin: $(origin), pos: '^-2 ^2 ^1'}
$execute at @n[nbt={UUID:$(origin)}] run \
  function ww:config/board/action/pool/_update_sign {id: 'krampus', origin: $(origin), pos: '^-2 ^1 ^1'}
...
# Center
$execute at @n[nbt={UUID:$(origin)}] run \
  function ww:config/board/action/pool/_update_sign {id: 'bewitcher', origin: $(origin), pos: '^ ^2 ^1'}
$execute at @n[nbt={UUID:$(origin)}] run \
...
# Right 1
$execute at @n[nbt={UUID:$(origin)}] run \
  function ww:config/board/action/pool/_update_sign {id: 'rigger', origin: $(origin), pos: '^1 ^2 ^1'}
$execute at @n[nbt={UUID:$(origin)}] run \
...
# Right 2
$execute at @n[nbt={UUID:$(origin)}] run \
  function ww:config/board/action/pool/_update_sign {id: 'phrenapates', origin: $(origin), pos: '^2 ^2 ^1'}
$execute at @n[nbt={UUID:$(origin)}] run \
  function ww:config/board/action/pool/_update_sign {id: 'aaa', origin: $(origin), pos: '^2 ^1 ^1'}

$execute at @n[nbt={UUID:$(origin)}] run data merge block ^2 ^ ^1 { \
  front_text:{has_glowing_text:0b, messages: ['{"text":""}', \
  '{"text":"重置怨灵池设定","color":"red","bold":true,"clickEvent":{"action":"run_command","value":"/function ww:config/board/action/_run {func: \\"ww:config/board/action/pool/_reset_campaign\\", args: {campaign: \\"spirits\\", page: \\"_spirits\\"}}"}}', \
  '{"text":""}', \
  '{"text":""}'], color:"white"} \
}
$execute at @n[nbt={UUID:$(origin)}] run data merge block ^2 ^-1 ^1 { \
