...
$execute if score %length ww.var matches 1.. run \
  data modify storage ww:output roleJourneyFormatted append value '{"text":"$(name)","color":"aqua","bold":true,"hoverEvent":{"action":"show_text",value:[{"text":"源自於: "},{"text":"$(from)","color":"green","bold":true}]}}'

$execute unless score %length ww.var matches 1.. run \
  data modify storage ww:output roleJourneyFormatted append value '{"text":"$(name)","color":"aqua","bold":true,"hoverEvent":{"action":"show_text",value:[{"text":"初始職業","color":"red","bold":true}]}}'
