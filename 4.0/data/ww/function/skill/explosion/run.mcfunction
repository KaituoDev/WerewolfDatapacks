...
execute as @a run function ww:chat/_dm { type: 'rich_info', \
  message: '[{"selector":"@n[tag=this]","color":"red"},{"text":" 自爆了, 強制進入夜晚","color":"white","bold":false}]' \
}
tag @s remove this

function ww:player/dead
execute if entity @s[tag=ww.fool] as @a run function ww:chat/_dm {type: 'lmao', message: '沒事, 他是蠢的, 大家繼續投票'}
