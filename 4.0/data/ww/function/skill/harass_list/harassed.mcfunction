...
execute as @a[tag=hentai] run function ww:chat/_dm {type: 'rich_info', message: \
  '[{"selector":"@n[tag=this_hentai]","color":"red","bold":true},{"text":" 被 ","color":"white","bold":false},{"selector":"@n[tag=this]","color":"green","bold":true},{"text":" 騷擾了","color":"white","bold":false}]' \
}
