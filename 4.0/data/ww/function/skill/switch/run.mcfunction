...
function ww:chat/_dm {type: 'rich_info', message: '[{"text":"你使用了代罪 ","color":"white","bold":false},{"selector":"@n[tag=ww.switch.candidate]","color":"green","bold":true},{"text":" 交换位置","color":"white","bold":false}]'}

execute as @n[tag=ww.switch.candidate] run \
  function ww:chat/_dm {type: 'info', message: '你被代罪换过来了'}
