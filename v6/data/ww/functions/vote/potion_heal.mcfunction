...
tag @p[tag=currentTarget] add witchSave

execute if entity @a[tag=currentTarget] run tellraw @s [{"storage":"ww:game","nbt":"DISPLAY.info","interpret":true},{"text":"","bold":false},{"text":"你向 ","color":"white","bold":false},{"selector":"@p[tag=currentTarget]","color":"green","bold":true},{"text":" 投解藥","color":"white","bold":false}]
